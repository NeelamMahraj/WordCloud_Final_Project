...
def calculate_frequencies(file_contents):
    ...
    for words in file_contents.split():
        if words.isalpha() and words.lower() not in uninteresting_words:
            dict_list.append(words.lower())
    for words in dict_list:
        if words not in required_dict:
            required_dict[words] = 1
        required_dict[words] += file_contents.split().count(words)
    
    
    #wordcloud
    cloud = wordcloud.WordCloud()
    cloud.generate_from_frequencies(required_dict)
